...
const rawAccounts = fs.readFileSync('./accounts.txt').toString().split('\r\n')
const accounts = rawAccounts
  .map((accounts) => {
    const [email, password, genre, songWriterName] = accounts.split(':')
...
const checkSettings = async () => {
  ...
  if (checkForDuplicates(accounts.map((account) => account.email))) {
    console.log('***SETTINGS ERROR: DUPLICATE EMAIL***')
    return
  }
...
;(async () => {
  ...
  console.log('-----HANDLE ERROR DATA START-----')
  while (errorData.length > 0) {
    ...
    const data = getRandomFromArr(errorData, 1)[0]

    if (data.usedProxy) {
      options.push(`--proxy-server=${data.usedProxy}`)
  ...
  console.log('-----HANDLE ERROR DATA END-----')
